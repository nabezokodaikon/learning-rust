...
fn lex(input: &str) -> Result<Vec<Token>, LexError> {
    ...
    while pos < input.len() {
        match input[pos] {
            ...
            b'*' => lex_a_token!(lex_astersk(input, pos)),
...
fn lex_astersk(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
...
fn parse(tokens: Vec<Token>) -> Result<Ast, ParseError> {
    ...
    match tokens.next() {
        Some(tok) => Err(ParseError::RedundanExpression(tok)),
        None => Ok(ret),
    }
}
        ...
        .and_then(|tok| match tok.value {
            ...
            TokenKind::LParen => {
                ...
                match tokens.next() {
                    ...
                    Some(t) => Err(ParseError::RedundanExpression(t)),
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        use self::ParseError::*;
        match self {
            UnexpectedToken(tok) => write!(f, "{}: {} is expected", tok.loc, tok.value),
            NotExpression(tok) => write!(
                f,
                "{}: '{}' is not a start of expression",
                tok.loc, tok.value
            ),
            NotOperator(tok) => write!(f, "{}: '{}' is not operator", tok.loc, tok.value),
            UnclosedOpenParen(tok) => write!(f, "{}: '{}' is not closed", tok.loc, tok.value),
            RedundanExpression(tok) => write!(
                f,
        ...
        let (e, loc): (&StdError, Loc) = match self {
            ...
            Parser(e) => {
                let loc = match e {
                    ...
                    P::RedundanExpression(Token { loc, .. }) => Loc(loc.0, input.len()),
